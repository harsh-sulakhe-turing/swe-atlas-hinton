...
- **Stage** — *how/when* a check runs. Stage 1 is deterministic Python (no LLM,
  fail-fast). Stage 2 is semantic (LLM judgment).
...
| Q06 | **Factual correctness** — each criterion's claim is true at `base_commit` | reject | 🚧 in progress |
...
**Q06 (factual correctness)** — in progress — is the one check that needs more than
the bundle text: it verifies each criterion's factual claim against the actual
repository at `base_commit`. It runs the agent with a `run_bash` tool inside the
task's own faithful Docker container (built from `environment/Dockerfile`,
network-disabled, non-root, capability-dropped, ephemeral). A read-only source pass
(`Q06a`) and a sandboxed runtime pass (`Q06b`) independently re-derive the facts.
...
- ✅ **Stage 2 text checks (Q01–Q05, Q07–Q12)** — built, reviewed, live-calibrated.
  122 tests passing.
- 🚧 **Q06 factual correctness** — *in progress*. The container-build harness +
  `run_bash` tool + Q06 agent role.
...
autoqc/
  ...
  bundle.py              tolerant bundle loader
  ...
  agent/
    ...
    checks.py            SemanticCheck registry (Q01–Q05, Q07, Q08, Q10, Q11)
...
tests/                   122 unit tests (all offline via FakeLLMClient)
```
